...
export default class Boss extends TimeEffect {
    ...
    private sprintModeOne() {
        ...
        this.currentTween = cc
        .tween(this.node)
        .parallel(cc.tween().to(1, { x: randX}), 
                  cc.tween().to(1, { y: randY}))
        .parallel(cc.tween().to(moveTime, { x: this.player.x}, {easing: 'sineIn'}), 
                  cc.tween().to(moveTime, { y: this.player.y}, {easing: 'sineIn'}))
        .call(() => {this.spawnFallenRock();
    ...
    private throwWeapon() {
        ...
        this.currentTween = cc
            .tween(this.node)
            .parallel(cc.tween().to(moveTime, { x: posX}, {easing: 'quartOut'}), 
            cc.tween().to(moveTime, { y: posY }, {easing: 'quartOut'}))
            .call(() => {let newParticleSystem = cc.instantiate(this.axeParticleSystem);
    ...
    private pickupWeapon() {
        ...
        .tween(this.node)
        .delay(1)
        .parallel(cc.tween().to(moveTime, { x: this.weapon.x}), 
                  cc.tween().to(moveTime, { y: this.weapon.y}))
        .call(() => {this.weapon.destroy();
                     this.getWeapon();
    ...
    private postInvincible() {
        this.scheduleOnce(() => {this.isInvincible = false;}, 0.5); 
